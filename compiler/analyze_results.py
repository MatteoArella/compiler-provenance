if __name__ == '__main__':
    ...
    tester = ClassificationTester() \
                        .pipelines_load(pipelines_path='compiler-provenance/compiler/{}'.format(path), \
    ...
    dump(tester, 'compiler-provenance/compiler/tester.joblib', compress=('gzip', 6))
    reports = tester.classification_reports()
    dump(reports, 'compiler-provenance/compiler/reports.joblib', compress=('gzip', 6))
    dump(best_pipeline, 'compiler-provenance/compiler/bestmodel.joblib', compress=('gzip', 6))
    ...
    for classifier, predict, proba in zip(tester.classifiers_, tester.predictions_, tester.predictions_proba_):
        ...
        plt.savefig('compiler-provenance/images/comp/curves/{}-roc-curve.pdf'.format(classifier), bbox_inches='tight')
        skplt.metrics.plot_precision_recall(y_test, proba, title='')
        plt.savefig('compiler-provenance/images/comp/curves/{}-prec-recall-curve.pdf'.format(classifier), bbox_inches='tight')
        skplt.metrics.plot_confusion_matrix(y_test, predict, title='', normalize=True)
        plt.savefig('compiler-provenance/images/comp/{}-cm.pdf'.format(classifier), bbox_inches='tight')

    # get best n-gram range
    summary = load('compiler-provenance/compiler/{}/summary.joblib'.format(path))
